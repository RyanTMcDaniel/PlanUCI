...
if __name__ == "__main__":
    ...
    LOCKS      = {"COMPSCI161": "2027_fall", "MATH3A": "2027_winter"}

    # ── Step 1: validate locks ────────────────────────────────────────────────
    print("validate_locks:", LOCKS)
    v_valid, v_conflicts = validate_locks(LOCKS)
    print(f"  valid={v_valid}  conflicts={v_conflicts}")
    print()

    # ── Step 2: get a base plan to supply completed_courses ───────────────────
    print("Generating base plan for completed_courses seed...")
    try:
        base_gen = generate(MAJOR_ID, [], FIRST_GRAD)
    except FeasibilityError as e:
        FIRST_GRAD = f"{2028 + e.years_to_extend}_spring"
        base_gen   = generate(MAJOR_ID, [], FIRST_GRAD)

    # Use the base plan's CoursePlan with empty completed_courses for what-if
    base_plan = base_gen.variants[0].plan
    print(f"  Base plan: {sum(len(cs) for cs in base_plan.planned_courses.values())} courses\n")

    # ── Step 3: run what-if ───────────────────────────────────────────────────
    GRAD_Q = "2029_spring"
    print("=" * 60)
    print(f"run_whatif  major={MAJOR_ID}  grad={GRAD_Q}")
    print(f"  locks: {LOCKS}")
    print("=" * 60)
    ...
    result = run_whatif(
        ...
        major_id           = MAJOR_ID,
        graduation_quarter = GRAD_Q,
        units_per_quarter  = 16,
    )

    if result.lock_conflicts:
        print("Lock conflicts found — aborting:")
        for c in result.lock_conflicts:
            print(f"  {c}")
        raise SystemExit(1)

    print(f"\nQuarters available: {result.quarters_available}")
    print(f"Quarters needed:    {result.quarters_needed}")
    print(f"Tight timeline:     {result.tight_timeline}")
    print(f"Overflow omitted:   {result.overflow_count}")
    print()

    # ── Step 4: verify locked courses are pinned in all 3 variants ───────────
    print(f"{'Variant':<10}  {'soft_score':>10}  Locked courses pinned")
    print("-" * 68)

    all_pass = True
    for i, v in enumerate(result.variants, 1):
        pin_results: list[str] = []
        for course_id, expected_q in LOCKS.items():
            found_q = next(
                (q for q, cs in v.planned_courses.items() if course_id in cs),
                None,
            )
            ok = found_q == expected_q
            if not ok:
                all_pass = False
            pin_results.append(
                f"{course_id}→{found_q!r} [{'OK' if ok else 'FAIL'}]"
            )
        print(f"  {i:<8}  {v.soft_score:>10.4f}  {' | '.join(pin_results)}")

    print()
    print(f"All locked courses pinned across all variants: [{'PASS' if all_pass else 'FAIL'}]")

    # ── Step 5: print variant 1 quarter layout ────────────────────────────────
    print()
    print("Variant 1 quarter layout:")
    best = result.variants[0]
    locked_norm_set = {_norm(c) for c in LOCKS}
    for q in sorted(best.planned_courses.keys(), key=_qkey):
        cs = best.planned_courses[q]
        if not cs:
            continue
        locked_here  = [c for c in cs if _norm(c) in locked_norm_set]
        unlocked_here = [c for c in cs if _norm(c) not in locked_norm_set]
        tag = f"  [LOCKED: {locked_here}]" if locked_here else ""
        print(f"  {q}: {unlocked_here}{tag}")
