...
def _run_checks(plan: CoursePlan, client, expected_failures: set[str], label: str) -> bool:
    checks = [
        ("prereqs_satisfied",      prereqs_satisfied(plan, client)),
        ("major_requirements_met", major_requirements_met(plan, client)),
        ("units_valid",            units_valid(plan)),
    ...
    print(f"── {label} ──")
    for name, (passed, violations) in checks:
        should_fail = name in expected_failures
        caught = (not passed) == should_fail
        if not caught:
            all_ok = False
        status = "PASS" if caught else "FAIL"
        print(f"  [{status}] {name}: {'violation caught' if not passed else 'no violations'}")
...
if __name__ == "__main__":
    ...
    # ── Plan A: deliberate violations ─────────────────────────────────────────
    # 1. I&CSCI33 in fall 2024 but prereq (I&C SCI 32 / H32 / 32A) not satisfied.
    # 2. Spring 2025: 5 courses × 4 units = 20 units, exceeds 16-unit cap.
    plan_a = CoursePlan(
        major_id="BS-201A",
        completed_courses=["MATH2A"],
        planned_courses={
            "2024_fall":   ["I&CSCI33"],
            "2025_winter": ["I&CSCI32"],
            "2025_spring": ["MATH2B", "I&CSCI45C", "I&CSCI46",
                            "COMPSCI161", "IN4MATX43"],
        },
        units_per_quarter=16,
    )

    # ── Plan B: honors prereq — should pass prereqs_satisfied ─────────────────
    # I&C SCI H32 is an OR alternative to I&C SCI 32 for I&CSCI33.
    plan_b = CoursePlan(
        major_id="BS-201A",
        completed_courses=["I&C SCI H32"],
        planned_courses={
            "2024_fall": ["I&CSCI33"],
        },
        units_per_quarter=16,
    ...
    ok_a = _run_checks(
        plan_a, client,
        expected_failures={"prereqs_satisfied", "units_valid", "major_requirements_met"},
        label="Plan A (deliberate violations)",
    )

    ok_b = _run_checks(
        plan_b, client,
        # major_requirements_met will fail (incomplete plan); prereqs should pass
        expected_failures={"major_requirements_met"},
        label="Plan B (honors prereq — I&C SCI H32 satisfies I&CSCI33 prereq)",
    )

    print("Overall:", "ALL PASS" if ok_a and ok_b else "SOME CHECKS FAILED")
