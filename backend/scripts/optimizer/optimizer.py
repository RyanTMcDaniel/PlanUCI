...
if __name__ == "__main__":
    # All prereqs for the planned courses are satisfied via completed_courses:
    #   COMPSCI161  needs I&CSCI46, I&CSCI6B, I&CSCI6D, MATH2B
    #   COMPSCI143A needs I&CSCI46, I&CSCI51
    #   I&CSCI45C   needs I&CSCI33
    #   IN4MATX43   needs I&CSCI32
    #   STATS67     needs MATH2B
    # This makes every planned course freely moveable to any quarter, giving
    # the optimizer full freedom to improve the soft score via swaps.

    front_loaded = CoursePlan(
        major_id="BS-201A",
        completed_courses=[
            "I&CSCI32", "I&CSCI33", "I&CSCI46",
            "I&CSCI6B", "I&CSCI6D", "I&CSCI51", "MATH2B",
        ],
        planned_courses={
            "2024_fall":   ["COMPSCI161", "COMPSCI143A", "I&CSCI45C"],  # hard: avg ~6.25
            "2025_winter": ["IN4MATX43",  "STATS67",     "WRITING39A"], # easy: avg ~4.15
            "2025_spring": ["WRITING39B", "WRITING39C",  "BIOSCI93"],   # easy: avg ~4.68
        },
        units_per_quarter=16,
    )

    # ── evaluate ──────────────────────────────────────────────────────────────
    print("evaluate(front_loaded)")
    orig = evaluate(front_loaded)
    print(f"  valid:      {orig.valid}  (expected False — partial plan, major reqs incomplete)")
    if orig.violations:
        for v in orig.violations[:5]:
            print(f"    - {v}")
    print(f"  soft_score: {orig.soft_score:.4f}")
    col = 24
    for name, val in orig.soft_breakdown.items():
        print(f"    {name:<{col}} {val:.4f}")
    print()

    # ── optimize ──────────────────────────────────────────────────────────────
    print("optimize(front_loaded, max_iter=300, seed=42)")
    opt = optimize(front_loaded, max_iter=300, seed=42)
    print(f"  improved:   {opt.improved}")
    print(f"  soft_score: {opt.soft_score:.4f}  (was {orig.soft_score:.4f})")
    delta = orig.soft_score - opt.soft_score
    print(f"  delta:      {delta:+.4f}")
    print()

    print("Optimized plan:")
    for q in sorted(opt.plan.planned_courses.keys(), key=_qkey):
        courses = opt.plan.planned_courses[q]
        print(f"  {q}: {courses}")
    print()

    passed = opt.soft_score < orig.soft_score
    print(f"optimize() improves soft score: [{'PASS' if passed else 'FAIL'}]")
